/*
 * Alunos: Pedro Madeira, Thiago Teixeira e Luis Guedes
 * 
 * 
 */
...
public class Graph {
    ...
    // Verificar a existência de dois caminhos internamente disjuntos
    ...
    public void findDisjointPaths(int origem, int destino) {
        List<List<Integer>> todosCaminhos = new ArrayList<>();
        List<Integer> caminhoAtual = new ArrayList<>();
        boolean[] visitado = new boolean[V];

        findDisjointPathsUtil(origem, destino, visitado, caminhoAtual, todosCaminhos);

        // Quantidade de caminhos encontrados
    ...
    private void findDisjointPathsUtil(int u, int destino, boolean[] visitado, List<Integer> caminhoAtual, List<List<Integer>> todosCaminhos) {
        visitado[u] = true;
        caminhoAtual.add(u);
        ...
        if (u == destino) {
            ...
            for (int vizinho : adj[u]) {
                if (!visitado[vizinho]) {
                    findDisjointPathsUtil(vizinho, destino, visitado, caminhoAtual, todosCaminhos);
                }
            }
        }
    ...
    public List<Integer> findJoint() {
        ...
        for (int i = 0; i < V; i++) {
            if (!visitado[i]) {
                findJointDfs(i, visitado, disc, low, parent, articulacoes);
            }
        }
    ...
    private void findJointDfs(int u, boolean[] visitado, int[] disc, int[] low, int[] parent, List<Integer> articulacoes) {
        int children = 0;
        visitado[u] = true;
        disc[u] = low[u] = ++tempo;
        ...
        for (int w : adj[u]) {
            if (!visitado[w]) {
                children++;
                parent[w] = u;
                findJointDfs(w, visitado, disc, low, parent, articulacoes);
    ...
    public static void main(String[] args) throws Exception {
        ...
        Graph g = new Graph(Integer.parseInt(splitLine[0])+1);
        ...
        for(int i = 0; i < amountOfGraphs; i++) {
            ...
            g.addEdge(Integer.parseInt(splitLine[0]), Integer.parseInt(splitLine[1]));
        ...
        System.out.println("\n_______________________________________________________________________________________________\n");
        System.out.println("Articulações: " + g.findJoint());
        System.out.println("\n_______________________________________________________________________________________________");
